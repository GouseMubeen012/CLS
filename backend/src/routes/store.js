...
router.post('/register', authenticateToken, async (req, res) => {
  ...
  try {
    const { store_name, store_type, owner_name, mobile_number } = req.body;

    // Validate required fields
    if (!store_name || !store_type || !owner_name || !mobile_number) {
    ...
    const existingStore = await client.query(
      'SELECT * FROM stores WHERE mobile_number = $1',
      [mobile_number]
    );
    ...
    const storeResult = await client.query(
      ...
      [store_name, store_type, owner_name, mobile_number, hashedPin]
    );
    ...
    // Generate PDF with store credentials
    const pdfPath = path.join('uploads', `store_${store.store_id}_credentials.pdf`);
    ...
    // Add content to PDF
    doc.fontSize(25).text('Store Credentials', { align: 'center' });
    doc.moveDown();
    doc.fontSize(14);
    doc.text(`Store ID: ${store.store_id}`);
    doc.text(`Store Name: ${store.store_name}`);
    doc.text(`Owner Name: ${store.owner_name}`);
    doc.moveDown();
    doc.fontSize(16).text('Login Credentials', { underline: true });
    doc.fontSize(14);
    doc.text(`Mobile Number: ${store.mobile_number}`);
    doc.text(`PIN: ${pin}`);
    doc.moveDown();
    doc.fontSize(12).text('Important: Please keep this PIN secure. You will need it to log in to your store account.');
    ...
    // Return success with store details, PIN and PDF URL
    res.json({
      ...
      credentials: {
        mobile_number: store.mobile_number,
        pin: pin, // Send the unhashed PIN to be shown in PDF
        pdfUrl: `/${pdfPath}` // URL to download the PDF
      }
    });
...
router.post('/login', async (req, res) => {
  try {
    ...
    // Verify PIN
    const validPin = await bcrypt.compare(pin, store.pin_code);
    if (!validPin) {
      return res.status(401).json({ error: 'Invalid credentials' });
    }
...
router.get('/settlements/:storeId', authenticateToken, async (req, res) => {
  try {
    ...
    // Get store details
    const storeResult = await pool.query(
      `SELECT store_name, owner_name, mobile_number
