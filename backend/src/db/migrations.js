  ...
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS students (
        ...
        card_number INTEGER UNIQUE CHECK (card_number >= 10000 AND card_number <= 99999),
    ...
    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_students_gr_active 
      ON students(gr_number, is_active);
    `);

    // Create trigger to ensure only one active card per GR number
    await pool.query(`
      CREATE OR REPLACE FUNCTION check_active_card()
      RETURNS TRIGGER AS $$
      BEGIN
        IF NEW.is_active = TRUE THEN
          -- Deactivate any existing active cards for this GR number
          UPDATE students 
          SET is_active = FALSE 
          WHERE gr_number = NEW.gr_number 
    ...
    await pool.query(`
      DROP TRIGGER IF EXISTS ensure_single_active_card ON students;
      CREATE TRIGGER ensure_single_active_card
    ...
    await pool.query(`
      ...
      -- Add index on created_at for efficient cleanup of old transactions
